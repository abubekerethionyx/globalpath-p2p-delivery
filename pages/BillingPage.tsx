...
const BillingPage: React.FC<BillingPageProps> = ({ user }) => {
  ...
  return (
    <div className="space-y-6 animate-in pb-12 w-full">
      {/* Header Section */}
      <div className="flex flex-col lg:flex-row justify-between items-end gap-6 border-b border-slate-100 pb-6">
        <div>
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-black text-slate-900 tracking-tighter mb-1">
        ...
        {/* Toggle Grid */}
        <div className="bg-slate-100 p-1 rounded-xl flex font-bold text-[10px] uppercase tracking-widest shadow-inner">
          <button
            onClick={() => { setActiveTab('transactions'); setFilterStatus('ALL'); }}
            className={`px-6 py-2.5 rounded-lg transition-all ${activeTab === 'transactions'
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-lg transition-all ${activeTab === 'subscriptions'
              ? 'bg-white text-slate-900 shadow-sm'
              : 'text-slate-400 hover:text-slate-600'
              }`}
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-slate-900 p-5 rounded-2xl text-white shadow-xl relative overflow-hidden group col-span-2 lg:col-span-1">
          ...
          <p className="text-2xl md:text-3xl font-black mb-2">
            {totalValue.toLocaleString()} <span className="text-xs font-bold text-slate-500">ETB</span>
          </p>
          <div className="inline-flex items-center px-2 py-1 bg-white/10 rounded-lg text-[9px] font-bold">
            {activeTab === 'transactions' && isPicker ? 'Direct P2P Income' : 'Direct Payment'}
          </div>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-center">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">Count</p>
          <p className="text-2xl md:text-3xl font-black text-slate-900 mb-1">{displayedData.length}</p>
          <p className="text-[9px] font-medium text-slate-500">Matching Records Found</p>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-center">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">Avg. Value</p>
          <p className="text-2xl md:text-3xl font-black text-[#009E49] mb-1">{averageValue.toLocaleString(undefined, { maximumFractionDigits: 0 })} <span className="text-[10px] font-bold text-slate-300">ETB</span></p>
          <p className="text-[9px] font-medium text-slate-500">Per Transaction</p>
        </div>
      </div>
      ...
      <div className="bg-white p-2 rounded-2xl border border-slate-200 shadow-sm flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 bg-slate-50 px-2 py-1.5 rounded-xl">
          <span className="hidden sm:inline text-[9px] font-bold text-slate-400 uppercase">Status</span>
          <select
            value={filterStatus}
        ...
        <div className="flex items-center gap-2 bg-slate-50 px-2 py-1.5 rounded-xl">
          <span className="hidden sm:inline text-[9px] font-bold text-slate-400 uppercase">From</span>
          <input
            type="date"
        ...
        <div className="flex items-center gap-2 bg-slate-50 px-2 py-1.5 rounded-xl">
          <span className="hidden sm:inline text-[9px] font-bold text-slate-400 uppercase">To</span>
          <input
            type="date"
            value={dateRange.end}
        ...
        {(filterStatus !== 'ALL' || dateRange.start || dateRange.end) && (
          <button
            ...
            className="ml-auto text-[10px] font-bold text-red-500 hover:text-red-600 px-2"
          >
            Clear
          </button>
        )}
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em]">
              <tr>
                ...
                <th className="px-4 py-4 text-right">Amount (ETB)</th>
              ...
              {displayedData.length === 0 ? (
                ...
                displayedData.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-4 font-mono text-[10px] text-slate-500 font-bold group-hover:text-slate-900">
                        #{refId}
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-slate-900">{date.toLocaleDateString()}</span>
                          <span className="text-[10px] text-slate-400 font-semibold">{date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        {isSub ? (
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                              <span className="text-xs font-bold text-slate-700">{planName} Subscription</span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                              <span className="text-xs font-bold text-slate-700">{(item as ShipmentItem).category || 'Shipment'}</span>
                              <span className="text-[9px] text-slate-400 font-bold uppercase">{(item as ShipmentItem).pickupCountry} → {(item as ShipmentItem).destCountry}</span>
                            </div>
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-4 text-right">
                        <span className={`text-xs font-black ${isSub ? 'text-indigo-600' : 'text-[#009E49]'}`}>
