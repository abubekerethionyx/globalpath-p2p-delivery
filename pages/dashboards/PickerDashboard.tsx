...
const PickerDashboard: React.FC<PickerDashboardProps> = ({ user, publicSettings }) => {
    ...
    return (
        <div className="max-w-[1600px] mx-auto space-y-4 md:space-y-6 animate-in fade-in duration-700 pb-12 px-3 md:px-4">
            {/* Friendly Verification Prompt - Compact Mobile */}
            {user.verificationStatus !== VerificationStatus.VERIFIED && (
                <div className={`p-4 md:p-5 rounded-2xl md:rounded-3xl border-2 shadow-xl ${user.verificationStatus === VerificationStatus.PENDING
                    ? 'bg-gradient-to-br from-amber-50 to-orange-50 border-amber-200'
                    : 'bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200'
                    }`}>
                    <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 md:gap-6">
                        <div className="flex items-start gap-4 md:gap-6 flex-1">
                            <div className={`w-12 h-12 md:w-20 md:h-20 rounded-xl md:rounded-[1.5rem] flex items-center justify-center shadow-lg shrink-0 ${user.verificationStatus === VerificationStatus.PENDING
                                ? 'bg-gradient-to-br from-amber-400 to-orange-500 text-white'
                                : 'bg-gradient-to-br from-blue-500 to-indigo-600 text-white'
                                }`}>
                                {user.verificationStatus === VerificationStatus.PENDING ? (
                                    <svg className="w-6 h-6 md:w-10 md:h-10 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                ) : (
                            ...
                            <div className="flex-1">
                                <h3 className={`text-lg md:text-2xl font-black tracking-tight mb-1 md:mb-2 ${user.verificationStatus === VerificationStatus.PENDING ? 'text-amber-900' : 'text-blue-900'
                                    }`}>
                                    {user.verificationStatus === VerificationStatus.PENDING
                                        ? '⏳ Verification In Progress'
                                        : '🚀 Unlock Your Full Potential!'}
                                </h3>
                                <p className={`text-xs md:text-sm font-medium mb-3 md:mb-4 max-w-2xl ${user.verificationStatus === VerificationStatus.PENDING ? 'text-amber-800/80' : 'text-blue-800/80'
                                    }`}>
                                    {user.verificationStatus === VerificationStatus.PENDING
                                        ? 'We are reviewing your documents. You can browse meanwhile.'
                                        : 'Complete verification to start earning.'}
                                </p>

                                {user.verificationStatus === VerificationStatus.UNVERIFIED && (
                                    <div className="hidden md:block space-y-3">
                                        <p className="text-xs font-black text-blue-900 uppercase tracking-wider mb-2">✨ Benefits of Verification:</p>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div className="flex items-center gap-3 bg-white/60 backdrop-blur-sm px-4 py-3 rounded-xl border border-blue-100">
                                                <div className="w-8 h-8 rounded-lg bg-green-500 flex items-center justify-center flex-shrink-0">
                                                    <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                                                </div>
                                                <span className="text-xs font-bold text-slate-700">Accept requests</span>
                                            </div>
                                            <div className="flex items-center gap-3 bg-white/60 backdrop-blur-sm px-4 py-3 rounded-xl border border-blue-100">
                                                <div className="w-8 h-8 rounded-lg bg-green-500 flex items-center justify-center flex-shrink-0">
                                                    <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                                                </div>
                                                <span className="text-xs font-bold text-slate-700">Earn rewards</span>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                {user.verificationStatus === VerificationStatus.PENDING && (
                                    <div className="flex items-center gap-2 bg-white/60 backdrop-blur-sm px-3 py-2 md:px-4 md:py-3 rounded-xl border border-amber-200 w-fit">
                                        <div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></div>
                                        <span className="text-[10px] md:text-xs font-bold text-amber-800">Review time: 24-48h</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {user.verificationStatus === VerificationStatus.UNVERIFIED && (
                            <div className="flex flex-col gap-3 lg:items-end w-full lg:w-auto">
                                <button
                                    ...
                                    className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 md:px-10 md:py-4 rounded-xl md:rounded-2xl font-black uppercase text-[10px] md:text-xs tracking-widest hover:from-blue-700 hover:to-indigo-700 shadow-xl shadow-blue-200 transition-all flex items-center justify-center gap-3 group w-full lg:w-auto"
                                >
            ...
            {/* Profile Integrity Audit / Trust Score - Compact Mobile */}
            {user.verificationStatus === VerificationStatus.VERIFIED && (
                (!user.homeAddress || !user.nationalId && !user.passportNumber || !user.emergencyContact) && (
                    <div className="p-3 md:p-5 rounded-2xl md:rounded-3xl bg-indigo-50 border-2 border-indigo-100 flex flex-col md:flex-row items-center justify-between shadow-lg animate-in slide-in-from-top duration-700 gap-3 md:gap-6">
                        <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
                            <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-xl md:rounded-2xl flex items-center justify-center shadow-sm text-indigo-600 border border-indigo-50 shrink-0">
                            ...
                            <div>
                                <h3 className="text-sm md:text-lg font-black text-indigo-950 uppercase tracking-tight">Boost Trust Score</h3>
                                <p className="text-[10px] md:text-sm font-medium text-indigo-900/60 mt-0.5 md:mt-1 max-w-lg">
                                    Complete profile details for better visibility.
                                </p>
                            </div>
                        </div>
                        <button
                            onClick={() => navigate('/registration')}
                            className="bg-indigo-600 text-white px-6 py-2 md:px-8 md:py-4 rounded-xl md:rounded-2xl font-black uppercase text-[9px] md:text-[10px] tracking-widest hover:bg-slate-900 shadow-xl shadow-indigo-200 transition-all hover:-translate-y-1 w-full md:w-auto text-center"
                        >
                            Update
                        </button>
                    </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-3 md:gap-4">
                <div className="space-y-0.5 md:space-y-1">
                    <p className="text-[#009E49] text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em]">Logistics Command</p>
                    <h1 className="text-xl md:text-3xl lg:text-4xl font-black text-slate-900 tracking-tight">Picker Dashboard</h1>
                    <p className="text-slate-500 font-medium text-xs md:text-base">Partner Node: <span className="text-slate-900 font-bold">{user.firstName} {user.lastName}</span></p>
                </div>
                <button
                    onClick={() => navigate('/marketplace')}
                    className="bg-slate-900 text-white px-4 py-2.5 md:px-6 md:py-3 rounded-xl md:rounded-2xl font-black uppercase text-[9px] md:text-[10px] tracking-widest hover:bg-indigo-600 transform hover:-translate-y-1 transition-all shadow-xl shadow-slate-200 flex items-center gap-2 md:gap-3 w-full md:w-auto justify-center"
                >
                    <svg className="w-3.5 h-3.5 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    Discover Deliveries
                </button>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                <div className="bg-white p-3 md:p-5 rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm group hover:border-amber-500 transition-all">
                    <div className="flex justify-between items-start mb-2 md:mb-3">
                        <div className="p-1.5 md:p-2 bg-amber-50 rounded-lg md:rounded-xl text-amber-500">
                    ...
                    <p className="text-slate-400 text-[8px] md:text-[9px] font-black uppercase tracking-widest truncate">Applications</p>
                    <p className="text-xl md:text-3xl font-black text-slate-900 mt-0.5 md:mt-1">{pendingCount}</p>
                </div>
                ...
                <div className="bg-white p-3 md:p-5 rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <p className="text-xl md:text-3xl font-black text-slate-900 mt-0.5 md:mt-1">{lockedCount + transitCount}</p>
                </div>

                <div className="bg-slate-900 p-3 md:p-5 rounded-2xl md:rounded-3xl shadow-xl text-white">
                    <p className="text-slate-400 text-[8px] md:text-[9px] font-black uppercase tracking-widest truncate">My Revenue</p>
                    <div className="flex items-baseline gap-1 md:gap-2 mt-0.5 md:mt-1">
                        <p className="text-xl md:text-3xl font-black">{totalEarned.toLocaleString()}</p>
                        <p className="text-[8px] md:text-[10px] font-bold opacity-50">ETB</p>
                    </div>
                </div>

                <div className="bg-white p-3 md:p-5 rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm hover:border-[#009E49] transition-all cursor-pointer group" onClick={() => navigate('/my-travels')}>
                    <div className="flex justify-between items-start mb-2 md:mb-3">
                        <div className="p-1.5 md:p-2 bg-green-50 rounded-lg md:rounded-xl text-green-500 group-hover:bg-[#009E49] group-hover:text-white transition-all">
                        ...
                        <svg className="w-3 h-3 text-slate-300 group-hover:text-[#009E49] transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M9 5l7 7-7 7" /></svg>
                    </div>
                    <p className="text-slate-400 text-[8px] md:text-[9px] font-black uppercase tracking-widest truncate">Manage</p>
                    <p className="text-xl md:text-3xl font-black text-slate-900 mt-0.5 md:mt-1">Travels</p>
                </div>
            </div>
            ...
            <div className="overflow-x-auto pb-4 -mb-4">
                <div className="flex gap-2 bg-slate-100 p-1.5 rounded-xl w-fit">
                    <button
                        ...
                        className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === 'APPLICATIONS' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <span className={`w-1.5 h-1.5 rounded-full ${pendingCount > 0 ? 'bg-amber-500 animate-pulse' : 'bg-slate-300'}`}></span>
                        Apps ({pendingCount})
                    </button>
                    <button
                        onClick={() => setActiveTab('REQUESTS')}
                        className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'REQUESTS' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Action ({requestedCount})
                    </button>
                    <button
                        onClick={() => setActiveTab('LOCKED')}
                        className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'LOCKED' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Locked ({lockedCount})
                    </button>
                    <button
                        onClick={() => setActiveTab('TRANSIT')}
                        className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'TRANSIT' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Transit ({transitCount})
                    </button>
                    <button
                        onClick={() => setActiveTab('COMPLETED')}
                        className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'COMPLETED' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
