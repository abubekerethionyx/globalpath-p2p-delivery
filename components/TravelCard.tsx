...
const TravelCard: React.FC<TravelCardProps> = ({ travel, onPinClick, currentUser, isPinned = false }) => {
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden group">
            <div className="p-4">
                <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <div className={`shrink-0 px-2 py-1 rounded-md text-[8px] font-black uppercase tracking-widest ${isExpired || travel.status === 'COMPLETED' || travel.status === 'CANCELLED' ? 'bg-slate-100 text-slate-400' : 'bg-green-50 text-[#009E49]'}`}>
                        {travel.status === 'CANCELLED' ? 'Cancelled' : (isExpired || travel.status === 'COMPLETED') ? 'Completed' : 'Upcoming'}
                    </div>
                </div>

                <div className="space-y-3">
                    <div className="flex items-center gap-4 py-2 px-3 bg-slate-50 rounded-xl border border-slate-100 relative">
                        <div className="flex-1">
                            <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Origin</p>
                            <p className="text-xs font-bold text-slate-900">{travel.origin_country}</p>
                        </div>
                        <div className="flex items-center justify-center p-1.5 bg-white rounded-full shadow-sm border border-slate-100 absolute left-1/2 -translate-x-1/2">
                            <svg className="w-3 h-3 text-[#009E49]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                            </svg>
                        </div>
                        <div className="flex-1 text-right">
                            <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Destination</p>
                            <p className="text-xs font-bold text-slate-900">{travel.destination_country}</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                        <div className="p-2 bg-white border border-slate-100 rounded-lg">
                            <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Travel Date</p>
                            <p className="text-[10px] font-bold text-slate-900">
                                {new Date(travel.travel_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                            </p>
                        </div>
                        <div className="p-2 bg-white border border-slate-100 rounded-lg">
                            <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Capacity</p>
                            <p className="text-[10px] font-bold text-slate-900">{travel.weight_capacity ? `${travel.weight_capacity} kg` : 'Flexible'}</p>
                        </div>
                    </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 cursor-pointer hover:opacity-70 transition-opacity" onClick={toggleExpand}>
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="px-2 py-1.5 md:px-3 md:py-2 bg-slate-100 text-slate-900 rounded-lg text-[8px] md:text-[9px] font-black uppercase tracking-widest hover:bg-slate-200 transition"
                        >
                            Inspect
                        </button>
                        {currentUser?.role !== UserRole.PICKER && (
                            <button
                                ...
                                className={`px-2 py-1.5 md:px-4 md:py-2 rounded-lg text-[8px] md:text-[9px] font-black uppercase tracking-widest transition shadow-lg ${isPinned
                            ...
                            >
                                ...
                                ) : (travel.status === 'COMPLETED' || travel.status === 'CANCELLED' || isExpired) ? 'Closed' : 'Pin Item'}
                            </button>
                        )}
                    </div>
                </div>
                ...
                {expanded && (
                    <div className="mt-4 space-y-3 animate-in fade-in slide-in-from-top-2 border-t border-slate-50 pt-4">
                        {loadingPins ? (
